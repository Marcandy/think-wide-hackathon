...
export class QCaseRegistry {
	...
	register(
		caseId: string,
		input: { expected: string; nextAction?: string },
	): Receipt {
		if (this.#rows.has(caseId))
			throw new Error(`duplicate q-case registration: ${caseId}`);
...
export function registerAndrewCases(registry: QCaseRegistry): QCaseRegistry {
	...
	registry.register("Q06", {
		expected:
			"Late proposal at revision N after a human correction at N+1 is rejected; reopened view is correct",
		nextAction: "T11",
	});
