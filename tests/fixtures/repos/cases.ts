...
export type ExpectedOutcome =
	...
	| {
			...
			readonly reason: "stale_ref" | "missing_blob";
	  }
	/** Malformed or escaping. Rejected before any read. */
	| {
			readonly kind: "rejected";
			readonly reason: "path_escape" | "path_injection";
	  };
...
function windowOf(
	...
	const bytes = Buffer.from(fixtureFile(path).content, "utf8").subarray(
		startByte,
		endByte,
	);
	return Object.freeze({
...
export const BYTE_CASES: readonly ByteCase[] = Object.freeze([
	Object.freeze({
		...
		rationale:
			"Positive control for the byte path, proved before any boundary case.",
	}),
	Object.freeze({
		...
		rationale:
			"A bounded window returns exactly those bytes, not the whole file or a padded line.",
	}),
	Object.freeze({
		...
		rationale:
			"A byte window may split a multi-byte character; the result must still be exact.",
	}),
	Object.freeze({
		...
		rationale:
			"Same window, CRLF encoding. Must differ from LF - proves byte, not line, addressing.",
	}),
	Object.freeze({
		...
		rationale:
			"A window ending mid-character must return the partial bytes exactly, not repair them.",
	}),
	Object.freeze({
		id: "stale-branch-ref",
		qcase: "Q04",
		...
		expected: Object.freeze({
			...
			reason: "stale_ref" as const,
		}),
		rationale:
			"A superseded revision must report unavailable, never silently resolve to current HEAD.",
	}),
	Object.freeze({
		id: "missing-blob",
		qcase: "Q04",
		...
		expected: Object.freeze({
			kind: "unavailable" as const,
			reason: "missing_blob" as const,
		}),
		rationale:
			"An absent object is unavailable - distinct from denied and distinct from empty.",
	}),
	Object.freeze({
		id: "symlink-escape",
		qcase: "Q04",
		...
		rationale:
			"A symlink resolving outside the snapshot root is rejected before any read.",
	}),
]);
...
export const PATH_INJECTION_INPUTS: readonly {
	...
	Object.freeze({
		id: "dotdot-relative",
		path: "../etc/passwd",
		rationale: "Classic parent traversal.",
	}),
	Object.freeze({
		id: "dotdot-nested",
		path: "src/../../etc/passwd",
		rationale: "Traversal after a valid prefix.",
	}),
	Object.freeze({
		id: "absolute-posix",
		path: "/etc/passwd",
		rationale: "Absolute path ignores the snapshot root.",
	}),
	Object.freeze({
		id: "absolute-windows",
		path: "C:\\Windows\\win.ini",
		rationale: "Drive-qualified absolute path.",
	}),
	Object.freeze({
	...
	Object.freeze({
		id: "git-internal",
		path: ".git/config",
		rationale: "Repository internals are not selected source.",
	}),
]);
...
export function restoreFixtureRepo(repo: RepoName, dest: string): string {
	...
	execFileSync("git", ["clone", "--quiet", bundlePath(repo), target], {
		stdio: "pipe",
	});
	return target;
}
