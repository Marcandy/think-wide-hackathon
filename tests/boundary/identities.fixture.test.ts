...
describe("A/B identity fixtures", () => {
	...
	it("keeps A and B grants fully disjoint", () => {
		...
		expect(
			grantsFor("principal-b").some((g) => g.sourceId === GRANT_A.sourceId),
		).toBe(false);
		expect(
			grantsFor("principal-a").some((g) => g.sourceId === GRANT_B.sourceId),
		).toBe(false);
	});

	it("gives the two principals distinct subjects under one trusted issuer", () => {
		expect(new Set(PRINCIPALS.map((p) => p.subject)).size).toBe(
			PRINCIPALS.length,
		);
...
describe("independent access expectations", () => {
	it("orders the positive controls before every denial case", () => {
		...
		const firstDenial = ACCESS_EXPECTATIONS.findIndex(
			(e) => e.expected === "deny",
		);
		const lastAllow = ACCESS_EXPECTATIONS.map((e) => e.expected).lastIndexOf(
			"allow",
		);
	...
	it("never lets a denial case reference only objects the actor already holds", () => {
		for (const e of DENIAL_EXPECTATIONS.filter(
			(e) => e.kind === "foreign-id-substitution",
		)) {
			const held = grantsFor(e.principal);
			const ownsSource = held.some((g) => g.sourceId === e.presentedSourceId);
			const ownsInvestigation = held.some(
				(g) => g.investigationId === e.presentedInvestigationId,
			);
			expect(
				ownsSource && ownsInvestigation,
				`${e.id} must reference a foreign object`,
			).toBe(false);
		}
	});
	...
	it("points every positive control at objects the actor genuinely holds", () => {
		for (const e of POSITIVE_CONTROLS) {
			const held = grantsFor(e.principal);
			expect(
				held.some((g) => g.sourceId === e.presentedSourceId),
				e.id,
			).toBe(true);
			expect(
				held.some((g) => g.investigationId === e.presentedInvestigationId),
				e.id,
			).toBe(true);
		}
	});
