...
import { EffectLedger, digestArguments } from '../helpers/effects.ts'
...
describe('delayed provider stub', () => {
	it('completes within the timeout', async () => {
		...
		provider.enqueue({ latencyMs: 10, result: { kind: 'completed', body: 'ok' } })
	...
	it('distinguishes a declared failure from an unknown outcome', async () => {
		const ledger = new EffectLedger()
		const provider = createDelayedProvider({ ledger, timeoutMs: 100 })
		provider.enqueue({ latencyMs: 1, result: { kind: 'failed', error: 'upstream 500' } })
	...
	it('digests arguments and keeps the payload out of the ledger', async () => {
		...
		expect(JSON.stringify(dispatches)).not.toContain('SYNTHETIC_PROMPT_BODY_0001')
	})
