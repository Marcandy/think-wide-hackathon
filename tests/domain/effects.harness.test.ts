...
describe('effect ledger', () => {
	it('tracks writes, jobs and dispatches on separate channels', () => {
		...
		ledger.recordWrite({ table: 'investigations', op: 'insert', docId: 'doc_1' })
		ledger.recordJob({ jobId: 'job_1', reference: 'internal.analyze', runAtMs: 0 })
	...
	it('sees a job scheduled with no write - the denied-read-that-enqueued case', () => {
		const ledger = new EffectLedger()
		const before = ledger.snapshot()
		ledger.recordJob({ jobId: 'job_leak', reference: 'internal.analyze', runAtMs: 0 })
...
describe('argument digest', () => {
	it('is stable across key order', () => {
		expect(digestArguments({ a: 1, b: [2, 3] })).toBe(digestArguments({ b: [2, 3], a: 1 }))
	})
	...
	it('never retains the payload', () => {
		...
		const ledger = new EffectLedger()
		ledger.recordDispatch({ provider: 'p', model: 'm', args: { prompt: secret } })
...
describe('instrumentation wrappers', () => {
	it('records db writes without changing the return value', () => {
		...
		expect(snap.writes).toMatchObject([
			{ table: 'investigations', op: 'insert', docId: 'doc_inserted', outcome: 'committed' },
			{ table: 'unknown', op: 'patch', docId: 'doc_inserted', outcome: 'committed' },
		])
	})
	...
	it('records scheduled jobs by opaque id only', async () => {
		...
		await wrapped.runAfter(0, 'internal.analyze', { secret: 'SYNTHETIC_JOB_ARG_0001' })
