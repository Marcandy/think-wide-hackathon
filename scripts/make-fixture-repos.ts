...
function buildAlpha(root: string): { head: string; stale: string; symlinkSupported: boolean } {
	...
	const head = commit(root, 'alpha: current synthetic revision with unicode and escape link')
...
function buildBeta(root: string): { head: string } {
	initRepo(root)
	write(root, '.gitattributes', REPO_GITATTRIBUTES)
	write(root, 'README.md', '# SYNTHETIC FIXTURE REPO: beta\n\nOperator-authored. Not real source.\n')
	write(root, 'lib/beta.ts', BETA_TS)
	return { head: commit(root, 'beta: synthetic revision') }
}

function filesFor(repo: RepoName, root: string): RepoManifest['repos'][RepoName]['files'] {
...
function main(): void {
	...
	try {
		...
		console.log('wrote tests/fixtures/repos/{alpha,beta}.bundle and manifest.json')
	} finally {
